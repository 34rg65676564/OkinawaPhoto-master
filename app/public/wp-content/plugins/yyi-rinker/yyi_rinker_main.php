...
ORDER BY {$wpdb->posts}.ID ASC", $values));
	...
	public function image_html( $meta_datas, $shop_type, $atts = [] ) {
		...
		if ($this->is_some_images && is_array($v_images)) {
			...
			$html .= '<img class="yyi-rinker-images-arrow yyi-rinker-images-arrow-left hidden" width="30" heigth="30" src="' . esc_url($this->arrow_left_img_url) . '">';
			$is_first = true;
			...
			if (count( $v_images ) > 0) {
				$html .= '<img class="yyi-rinker-images-arrow yyi-rinker-images-arrow-right" width="30" heigth="30" src="' . esc_url( $this->arrow_right_img_url ) . '">';
			}
			$html .= '</div>';
...
class Yyi_Rinker_Plugin_Gutenberg extends Yyi_Rinker_Abstract_Base
{
	...
	function gutenberg_rinker_register_block()
	{
		...
		wp_add_inline_script(
			'gutenberg-rinker',
			sprintf(
				'var gutenberg_rinker = { localeData: %s, admin_url: %s , designs: %s };',
